...
### Success Criteria
- [x] Local: `bash scripts/setup.sh --check` → exit 0 with per-item OK/MISSING lines on this machine. ✓ `OK python venv + dependencies / OK ollama reachable / OK model qwen2.5:3b available`, check-exit=0
- [x] End-to-end: `make smoke` → `SMOKE PASS` (task completed, file produced in smoke workspace) within the cost-model budget; `SMOKE SKIP` only if ollama is down (documented). ✓ `SMOKE PASS: task finished and hello.txt created` (after the Phase 4 amendment; well under the 15-min budget)
- [x] End-to-end: `make check` → green after all changes. ✓ flake8 clean, mypy `Success: no issues found in 16 source files`, `62 passed`
- [x] Manual: GitHub Pages URL loads with the landing page. ✓ Pages enabled via API (`source: main /docs`); `curl -sI https://nitishagar.github.io/diy-manus/` → `HTTP/2 200`; page `<title>DIY Manus — a local-first general agent</title>`; Deploy Pages workflow green.
...
## Amendments
- AMENDED 2026-09-19 Phase 4 [factual]: smoke task reworded to state the finish instruction explicitly ("Create a file named hello.txt whose content is exactly: hi. Then call finish.") and raised to --max-steps 8; one system-prompt clause added ("As soon as the user's goal is met, call finish…"). First smoke run failed: qwen2.5:3b created hello.txt with "hi" on step 1 but never called finish, spending all 4 steps on near-duplicate calls (distinct args, so the identical-call guard correctly did not fire). No mechanism change. — evidence: smoke stderr trace 2026-09-19 in HANDOFF_LEDGER.md.
- AMENDED 2026-09-19 Phase 3 [factual]: default MANUS_MODEL changed qwen3:4b → qwen2.5:3b, and the system prompt + tool descriptions compacted (payload 939 → 480 prompt tokens) — the plan's premise misstated the environment: on this CPU-only machine qwen3:4b's thinking mode cannot be suppressed via ollama 0.21.2 /v1 when tools are present (matches ollama issue #17969), so a step call took 181–197 s and often emitted no tool call at all; prompt-eval also scales at ~5 tok/s. Measured: qwen2.5:3b emits clean tool calls at ~50 s/step with the compact payload. Same seam, same loop; no mechanism change. — evidence: timed /v1 runs 2026-09-19 recorded in HANDOFF_LEDGER.md.
